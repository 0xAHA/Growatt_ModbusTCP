...
!!! warning "This extraction drops the Range column"
    The source workbook has a **Range** column that this page does not carry, and it is the
    only place V1.39 says a register may go negative — 53 registers use it, such as
    `-15~15A` for string currents. Sign information is therefore **absent from this page**,
    and its absence here is not evidence that a register is unsigned.

    Note also that section 2 of the source states: *"It is 16 bits (two bytes) unsigned
    integer for each holding and input register."* Unsigned is the documented default, so
    every `signed` flag in a profile is a departure from it.

    For sign questions use `Protocols/protocol_register_review.xlsx`, generated by
    `build_protocol_workbook.py` straight from the vendor workbooks, which carries every
    source column beside what the profiles currently do.
