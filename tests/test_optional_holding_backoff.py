...
def test_availability_map_covers_every_vpp_block_control():
    ...
    #
    # DERIVED from the register addresses rather than written out. The hard-coded version
    # of this list omitted `vpp_ac_charge_enable` (30410) - the same omission it was
    # written to catch, in the map and in the test together, so the test passed and the
    # control stayed ungated on both platforms. A list that has to be kept in step with
    # the thing it checks is only ever as good as the moment it was typed.
    blocks = {
        30100: 'vpp_control_authority_available',
        30200: 'vpp_export_limit_available',
        30201: 'vpp_export_limit_available',
        30407: 'vpp_remote_power_available',
        30408: 'vpp_remote_power_available',
        30409: 'vpp_remote_power_available',
        30410: 'vpp_remote_power_available',
    }

    expected = {
        name: blocks[definition['register']]
        for name, definition in _const.WRITABLE_REGISTERS.items()
        if definition.get('register') in blocks
    }

    assert flags == expected, (
        "a control on one of the optional VPP blocks is not gated on its availability "
        "flag, so when the block is skipped its entity publishes the dataclass default "
        "as though the inverter had reported it"
    )
